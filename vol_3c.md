...
- **제어 레지스터 — Control registers** 
  **CR0**, **CR3**, **CR4** (각 64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트).

- **디버그 레지스터 — Debug register** 
  **DR7** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트).
...
- 레지스터 **CS**, **SS**, **DS**, **ES**, **FS**, **GS**, **LDTR**, **TR** 각각에 대해 다음 필드:
  ...
  - **기준 주소 — Base address** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트). 
    **CS**, **SS**, **DS**, **ES**의 기준 주소 필드는 아키텍처상 정의된 비트가 32비트뿐이지만, **Intel 64** 아키텍처를 지원하는 프로세서에서는 해당 **VMCS** 필드도 64비트이다.
  
  - **세그먼트 한계 — Segment limit** (32비트). 한계 필드는 항상 **바이트** 단위이다.
  
  - **접근 권한 — Access rights** (32비트). 이 필드의 형식은 **표 26-2**에 있으며, 자세한 내용은 다음과 같다:
    - 하위 16비트는 64비트 세그먼트 디스크립터의 **상위 32비트 중 비트 23:8**에 대응한다. 
      코드·데이터 세그먼트 디스크립터의 **비트 19:16**은 세그먼트 한계의 상위 4비트에 대응하지만, 이 **VMCS** 필드의 해당 비트(**비트 11:8**)는 **예약**되어 있다.
  
    - **비트 16**은 **사용 불가 세그먼트 — unusable segment**를 나타낸다. 
      이런 세그먼트를 사용하려 하면 **64비트 모드**를 제외하고는 예외가 발생한다. 
      일반적으로 세그먼트 레지스터는 **널 셀렉터**로 적재되면 사용 불가가 된다.²
...
기준 주소, 세그먼트 한계, 접근 권한은 각 세그먼트 레지스터의 **“숨은” 부분 — hidden part**(또는 **“디스크립터 캐시 — descriptor cache”**)를 구성한다. 
세그먼트 레지스터의 디스크립터 캐시가 해당 셀렉터가 가리키는 메모리(**GDT** 또는 **LDT**) 안의 세그먼트 디스크립터와 **일치하지 않을 수 있기** 때문에, 이 데이터는 **VMCS**에 포함된다.

**SS**의 **DPL** 필드 값은 항상 논리 프로세서의 **현재 권한 수준 — CPL (Current Privilege Level)**과 같다.³

일부 프로세서에서는 **VMWRITE** 실행 시 **비트 11:8**이나 **비트 31:17**에 0이 아닌 값을 쓰려는 시도를 무시한다. 
이런 프로세서에서는 **VMREAD**가 해당 비트에 대해 항상 0을 반환하고, **VM 진입**은 해당 비트가 모두 0인 것처럼 다룬다(28.3.1.2절 참고).
