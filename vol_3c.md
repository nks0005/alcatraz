...
## 26.4.1 게스트 레지스터 상태(Guest Register State)

**게스트 상태 영역 — guest-state area**의 다음 필드는 프로세서 레지스터에 대응한다.

- **제어 레지스터 — Control registers** **CR0**, **CR3**, **CR4** (각 64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트).

- **디버그 레지스터 — Debug register** **DR7** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트).

- **RSP**, **RIP**, **RFLAGS** (각 64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트).¹

- 레지스터 **CS**, **SS**, **DS**, **ES**, **FS**, **GS**, **LDTR**, **TR** 각각에 대해 다음 필드:
  - **셀렉터 — Selector** (16비트).
  - **기준 주소 — Base address** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트). **CS**, **SS**, **DS**, **ES**의 기준 주소 필드는 아키텍처상 정의된 비트가 32비트뿐이지만, **Intel 64** 아키텍처를 지원하는 프로세서에서는 해당 **VMCS** 필드도 64비트이다.
  - **세그먼트 한계 — Segment limit** (32비트). 한계 필드는 항상 **바이트** 단위이다.
  - **접근 권한 — Access rights** (32비트). 이 필드의 형식은 **표 26-2**에 있으며, 자세한 내용은 다음과 같다:
    - 하위 16비트는 64비트 세그먼트 디스크립터의 **상위 32비트 중 비트 23:8**에 대응한다. 코드·데이터 세그먼트 디스크립터의 **비트 19:16**은 세그먼트 한계의 상위 4비트에 대응하지만, 이 **VMCS** 필드의 해당 비트(**비트 11:8**)는 **예약**되어 있다.
    - **비트 16**은 **사용 불가 세그먼트 — unusable segment**를 나타낸다. 이런 세그먼트를 사용하려 하면 **64비트 모드**를 제외하고는 예외가 발생한다. 일반적으로 세그먼트 레지스터는 **널 셀렉터**로 적재되면 사용 불가가 된다.²
    - **비트 31:17**은 **예약**되어 있다.
...
| 비트 위치 | 필드 |
|-----------|------|
| **3:0** | **세그먼트 유형 — Segment type** |
| **4** | **S — Descriptor type** (0 = 시스템; 1 = 코드 또는 data) |
| **6:5** | **DPL — Descriptor privilege level** |
| **7** | **P — Segment present** |
| **11:8** | **Reserved (예약)** |
| **12** | **AVL — Available for use by system software** |
| **13** | **Reserved (예약)** (**CS**를 제외하고는 예약) |
| **14** | **L — 64-bit mode active** (**CS**에만 해당) |
| **15** | **D/B — Default operation size** (0 = 16비트 세그먼트; 1 = 32비트 세그먼트) |
| **16** | **Segment unusable** (0 = 사용 가능; 1 = 사용 불가) |
| **31:17** | **Reserved (예약)** |

기준 주소, 세그먼트 한계, 접근 권한은 각 세그먼트 레지스터의 **“숨은” 부분 — hidden part**(또는 **“디스크립터 캐시 — descriptor cache”**)를 구성한다. 세그먼트 레지스터의 디스크립터 캐시가 해당 셀렉터가 가리키는 메모리(**GDT** 또는 **LDT**) 안의 세그먼트 디스크립터와 **일치하지 않을 수 있기** 때문에, 이 데이터는 **VMCS**에 포함된다.

**SS**의 **DPL** 필드 값은 항상 논리 프로세서의 **현재 권한 수준 — CPL (Current Privilege Level)**과 같다.³

일부 프로세서에서는 **VMWRITE** 실행 시 **비트 11:8**이나 **비트 31:17**에 0이 아닌 값을 쓰려는 시도를 무시한다. 이런 프로세서에서는 **VMREAD**가 해당 비트에 대해 항상 0을 반환하고, **VM 진입**은 해당 비트가 모두 0인 것처럼 다룬다(28.3.1.2절 참고).

- 레지스터 **GDTR**과 **IDTR** 각각에 대해 다음 필드:
  - **기준 주소 — Base address** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트).
  - **한계 — Limit** (32비트). 아키텍처에서는 이 필드가 16비트로만 규정되어 있지만, **VMCS**의 한계 필드는 32비트를 담는다.

- 다음 **MSR**들:
  - **IA32_DEBUGCTL** (64비트)
  - **IA32_SYSENTER_CS** (32비트)
  - **IA32_SYSENTER_ESP**와 **IA32_SYSENTER_EIP** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트)
  - **IA32_PERF_GLOBAL_CTRL** (64비트). 이 필드는 **“load IA32_PERF_GLOBAL_CTRL”** **VM 진입 제어**의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_PAT** (64비트). 이 필드는 **“load IA32_PAT”** **VM 진입 제어** 또는 **“save IA32_PAT”** **VM 종료 제어** 중 하나의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_EFER** (64비트). 이 필드는 **“load IA32_EFER”** **VM 진입 제어** 또는 **“save IA32_EFER”** **VM 종료 제어** 중 하나의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_BNDCFGS** (64비트). 이 필드는 **“load IA32_BNDCFGS”** **VM 진입 제어** 또는 **“clear IA32_BNDCFGS”** **VM 종료 제어** 중 하나의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_RTIT_CTL** (64비트). 이 필드는 **“load IA32_RTIT_CTL”** **VM 진입 제어** 또는 **“clear IA32_RTIT_CTL”** **VM 종료 제어** 중 하나의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_LBR_CTL** (64비트). 이 필드는 **“load guest IA32_LBR_CTL”** **VM 진입 제어** 또는 **“clear IA32_LBR_CTL”** **VM 종료 제어** 중 하나의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_S_CET** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트). 이 필드는 **“load CET state”** **VM 진입 제어**의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_INTERRUPT_SSP_TABLE_ADDR** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트). 이 필드는 **“load CET state”** **VM 진입 제어**의 1-설정을 지원하는 프로세서에서만 지원된다.
  - **IA32_PKRS** (64비트). 이 필드는 **“load PKRS”** **VM 진입 제어**의 1-설정을 지원하는 프로세서에서만 지원된다.

- **섀도 스택 포인터 레지스터 — shadow-stack pointer register** **SSP** (64비트; **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 32비트). 이 필드는 **“load CET state”** **VM 진입 제어**의 1-설정을 지원하는 프로세서에서만 지원된다.

- 레지스터 **SMBASE** (32비트). 이 레지스터는 논리 프로세서의 **SMRAM** 이미지 기준 주소를 담는다.

**참고:**

¹ 이 장에서는 **VMX 동작**을 지원하는 대부분의 프로세서가 **Intel 64** 아키텍처도 지원하므로, **RAX**, **RIP**, **RSP**, **RFLAGS** 등 프로세서 레지스터 표기를 쓴다. **Intel 64** 아키텍처를 지원하지 않는 프로세서에서는 이 표기는 해당 레지스터의 32비트 형(**EAX**, **EIP**, **ESP**, **EFLAGS** 등)을 가리킨다. 일부 위치에서는 표시된 레지스터의 **하위 32비트**만 가리키기 위해 **EAX** 같은 표기를 쓴다.

² 이 설명에는 예외가 몇 가지 있다. 예를 들어, 커밋 지점 이후 실패한 **태스크 스위치** 뒤에는 널이 아닌 셀렉터를 가진 세그먼트도 사용 불가가 될 수 있다(*Intel® 64 and IA-32 Architectures Software Developer’s Manual, Volume 3A*의 7.14절, “Exception and Interrupt Handling in 64-bit Mode”의 “Interrupt 10—Invalid TSS Exception (#TS)” 참고). 반면 **TR** 레지스터는 널 셀렉터를 갖더라도 프로세서 리셋 후에는 사용 가능하다(*Intel® 64 and IA-32 Architectures Software Developer’s Manual, Volume 3A*의 표 12-1 참고).

³ 보호 모드에서 **CPL**은 **CS** 셀렉터의 **RPL** 필드와도 연관된다. 그러나 **실주소 모드 — real-address mode**나 **가상-8086 모드 — virtual-8086 mode**에서는 **RPL** 필드는 의미가 없다.
